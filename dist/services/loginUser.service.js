...
function loginUserHelper(req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const { phone, password } = req.body;
            if (!phone || !password) {
                return res.status(statusCodes_js_1.StatusCodes.BAD_REQUEST).json({ message: statusCodes_js_1.StatusMessages[statusCodes_js_1.StatusCodes.BAD_REQUEST] });
            }
            const user = yield user_model_js_1.default.findOne({ phone: phone });
            if (!user) {
                return res.status(statusCodes_js_1.StatusCodes.BAD_REQUEST).json({ message: statusCodes_js_1.StatusMessages[statusCodes_js_1.StatusCodes.BAD_REQUEST] });
            }
            const isPasswordValid = bcryptjs_1.default.compare(password, user.password);
            if (!isPasswordValid) {
                return res.status(statusCodes_js_1.StatusCodes.UNAUTHORIZED).json({ message: statusCodes_js_1.StatusMessages[statusCodes_js_1.StatusCodes.UNAUTHORIZED] });
            }
